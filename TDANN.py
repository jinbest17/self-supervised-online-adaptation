...
class TD_Network(object):
    def __init__(self):
        self.X = tf.compat.v1.placeholder(tf.float32, [None,15], name='Placeholder_X')
        self.Y = tf.compat.v1.placeholder(tf.float32, [None,1], name='Placeholder_Y')
        
        with tf.compat.v1.variable_scope('TDANN'):
            self.Ypred = self.TD_ANN(self.X)
        
        self.lr = 1e-4
        self.global_step = tf.Variable(0, trainable = False)
        self.decay = tf.compat.v1.train.exponential_decay(self.lr, self.global_step, 100, 0.99, staircase = True)
        self.loss = tf.reduce_mean(input_tensor=tf.compat.v1.losses.mean_squared_error(self.Y, self.Ypred))
        self.optimizer = tf.compat.v1.train.AdamOptimizer(self.decay, name='Adam').minimize(self.loss)
...
def train(X_train1, Y_train1):
    conf = tf.compat.v1.ConfigProto(allow_soft_placement=True)
