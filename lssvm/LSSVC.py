...
class LSSVC():
    ...
    def __init__(self, gamma=1, kernel='rbf', l=10, dTh=0.7, **kernel_params): 
        # Hyperparameters
        self.gamma = gamma
        self.kernel_ = kernel
    ...
    def findMinIdx(self, x,  curr_time):
        minVal = 100000000
        minIdx = 0;
        for i in range(len(self.sv_x)):
            temp = x - self.sv_x[i]
            dist = np.dot(temp.T, temp)
            newVal = math.exp( self.t[i] / self.l) * math.sqrt(2 - 2 *math.exp( -1 * self.gamma * dist))
            #print(dist, newVal)
            if newVal < minVal:
                minVal = newVal
                minIdx = i
